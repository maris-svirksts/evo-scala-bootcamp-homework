...
object EffectsHomework1Declarative {
  ...
  final case class map[A](value: IO[A]) extends IO[A]
  final case class flatMap[A](value: IO[A]) extends IO[A]
  final case class *>[A](value: IO[A]) extends IO[A]
  final case class as[A](value: IO[A]) extends IO[A]
  final case class void[A](value: IO[A]) extends IO[A]
  final case class attempt[A](value: IO[A]) extends IO[A]
  final case class option[A](value: IO[A]) extends IO[A]
  final case class handleErrorWith[A](value: IO[A]) extends IO[A]
  final case class redeem[A](value: IO[A]) extends IO[A]
  final case class redeemWith[A](value: IO[A]) extends IO[A]
  final case class unsafeRunSync[A](value: IO[A]) extends IO[A]
  final case class unsafeToFuture[A](value: IO[A]) extends IO[A]
